    ...
    <div className="max-w-md mx-auto bg-chef-light-gray min-h-screen pb-20">
      ...
      <div className="bg-white mt-2 p-4">
        ...
        <div className="space-y-6">
          {weekPlan.map((day) => (
            <div key={day.date} className="border rounded-lg overflow-hidden card-hover">
              <div className="bg-chef-light-gray p-3 flex justify-between items-center">
                <div className="flex items-center">
                  <Calendar size={18} className="mr-2 text-chef-primary" />
              ...
              <div>
                {day.meals.length > 0 ? (
                  day.meals.map((meal) => (
                    <div 
                      key={meal.id}
                      className="flex items-center p-3 border-t hover:bg-gray-50 transition-colors"
                    >
                      <div
                        className="w-16 h-16 rounded-md bg-cover bg-center flex-shrink-0 cursor-pointer shadow-sm"
                        style={{ backgroundImage: `url(${meal.image})` }}
                        onClick={() => handleViewRecipe(meal.recipe_id)}
                      ></div>
                      
                      <div className="ml-3 flex-grow cursor-pointer" onClick={() => handleViewRecipe(meal.recipe_id)}>
                      ...
                      <div className="flex space-x-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="text-chef-medium-gray hover:text-chef-primary"
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="text-chef-medium-gray hover:text-red-500"
